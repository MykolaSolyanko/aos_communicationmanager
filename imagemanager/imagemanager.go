...
func New(cfg *config.Config, storage Storage, cryptoContext CryptoContext) (imagemanager *Imagemanager, err error) {
	...
	if cfg.SOTAServerURL != "" {
		if imagemanager.fileServer, err = fileserver.New(cfg.SOTAServerURL, cfg.ImageStoreDir); err != nil {
			return nil, aoserrors.Wrap(err)
		}
	}
...
func (imagemanager *Imagemanager) InstallService(serviceInfo cloudprotocol.ServiceInfo,
	...
	if err = fcrypt.DecryptAndValidate(fcrypt.DecryptParams{
		...
		CryptoContext:  imagemanager.cryptoContext,
	}); err != nil {
		return err
	}
	...
	if err = imagemanager.storage.AddService(ServiceInfo{
		ID:         serviceInfo.ID,
		AosVersion: serviceInfo.AosVersion,
		LocalURL:   createLocalURL(decryptedFile),
		RemoteURL:  remoteURL,
		Path:       decryptedFile,
