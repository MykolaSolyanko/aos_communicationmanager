...
type Controller struct {
	...
	cryptoContext fcrypt.CryptoContext
...
func New(config *config.Config, storage storage, certProvider CertificateProvider,
	cryptcoxontext *cryptutils.CryptoContext, decryptContext fcrypt.CryptoContext, insecure bool) (
	...
	umCtrl = &Controller{
		...
		componentDir:      path.Join(config.ComponentsStoreDir, "components"),
	...
	if umCtrl.fileServer, err = fileserver.New(config.FOTAServerURL, config.ComponentsStoreDir); err != nil {
		return nil, aoserrors.Wrap(err)
	}
	...
	allocator, err := spaceallocator.New(umCtrl.componentDir, 0, nil)
	if err != nil {
		return nil, aoserrors.Wrap(err)
	}

	umCtrl.allocator = allocator
...
func (umCtrl *Controller) UpdateComponents(
	...
	if currentState == stateIdle {
		...
		for _, component := range components {
			...
			if err = fcrypt.DecryptAndValidate(fcrypt.DecryptParams{
				...
				CryptoContext:  umCtrl.cryptoContext,
			}); err != nil {
				return umCtrl.currentComponents, aoserrors.Wrap(err)
			}
...
func (umCtrl *Controller) cleanupUpdateData() {
	...
	for _, entry := range entries {
		componentFile := filepath.Join(umCtrl.componentDir, entry.Name())
		if err := os.RemoveAll(componentFile); err != nil {
