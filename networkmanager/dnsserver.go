...
var (
	...
	ExecContext       = newExecShellCommander
)
...
func (dns *dnsServer) getAddress() string {
...
func (dns *dnsServer) addHosts(hosts []string, ip string) error {
	...
	for _, host := range hosts {
		for _, existHosts := range dns.hosts {
...
func (dns *dnsServer) restart() error {
	...
	return dns.restartProcess(process)
}
...
func (dns *dnsServer) start() error {
	...
	output, err := ExecContext(dns.binary, args...).CombinedOutput()
	if err != nil {
...
func (dns *dnsServer) restartProcess(pid *os.Process) error {
