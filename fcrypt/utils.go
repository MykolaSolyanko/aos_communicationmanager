...
func DecryptAndValidate(params DecryptParams) error {
	if err := decrypt(&params); err != nil {
		return err
	}

	if err := validateSigns(&params); err != nil {
		return err
	}
...
func decrypt(params *DecryptParams) (err error) {
	symmetricCtx, err := params.CryptoContext.ImportSessionKey(CryptoSessionKeyInfo{
	...
	srcFile, err := os.Open(params.EncryptedFile)
	if err != nil {
		return aoserrors.Wrap(err)
	}
	defer srcFile.Close()

	dstFile, err := os.OpenFile(params.DecryptedFile, os.O_RDWR|os.O_CREATE, 0o600)
	if err != nil {
		return aoserrors.Wrap(err)
	}
	defer dstFile.Close()
...
func validateSigns(params *DecryptParams) (err error) {
	signCtx, err := params.CryptoContext.CreateSignContext()
	if err != nil {
		return aoserrors.Wrap(err)
	}
	...
	file, err := os.Open(params.DecryptedFile)
	if err != nil {
		return aoserrors.Wrap(err)
	}
	defer file.Close()
